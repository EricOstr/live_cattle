...
# Your response can only be a single integer, either 0 or 1. 0 for No, 1 for Yes. Your output should be directly parsable by python code as an integer.
...
# for i in range(5):
#     text_chunk_queue.append({'role': 'user', 'content': chunks[curr_chunk]})
#     curr_chunk += 1
...
# release_date, month = get_date_info(text_chunk_queue)


# while curr_chunk < len(chunks):

#     print('Chunk: ', curr_chunk)
...
#             messages = text_chunk_queue.copy()
#             messages.append({
#                 'role':'user',
#                 'content': f"""
#                     If a table with the exact name "{table_info['name_in_report']}" is present in the previously given text, find the exact numeric value of the row "{row_info['name_in_report']}" in the table for the month {month.strftime('%B %Y')}.
#                     Note that you should strictly not refer to any other tables such as Federally Inspected Red Meat Production - United States.
#                     Only return the exact raw numeric value and nothing else, so that the rest of the python code can parse this output directly. If you cannot find the value, return double quotation marks signifying an empty string.
...
#         # if int(response):
#         #     print(f"\tTable {tables[table]['name_in_report']} present")


#     text_chunk_queue.pop(0)
#     text_chunk_queue.append({'role':'user', 'content':chunks[curr_chunk]})
